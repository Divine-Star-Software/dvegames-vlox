...
class Data {
    ...
    get2dDirection(vector) {
        vector.y = 0;
        vector.normalize();
        return Directions.FromVector(Vector2Like.Create(Math.round(vector.x), Math.round(vector.z)));
    }
    ...
    update() {
        ...
        this.forwardRay.origin.set(this.camera.globalPosition.x, this.camera.globalPosition.y, this.camera.globalPosition.z);
        this.forwardRay.direction.set(forwardDirection.x, forwardDirection.y, forwardDirection.z);
    }
}
export const CameraDirectionComponent = NCS.registerComponent({
    ...
    data: NCS.data(),
    init(component) {
        const cameraComponent = CameraProviderComponent.getRequired(component.node);
