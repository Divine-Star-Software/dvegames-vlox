...
export const CrossHairsComponent = NCS.registerComponent({
    ...
    init(comp) {
        ...
        const node = new TransformNode(`${comp.node.index}-cross-hairs-node`, context.data.scene);
        node.position.z = 10;
        node.parent = camera;
        const mat = new ShaderMaterial("shader", context.data.scene, {
            vertex: "crossHairs",
            fragment: "crossHairs",
        }, {
            attributes: ["position"],
        ...
        const vertical = CreateBox(`${comp.node.index}-cross-hairs-vertical`, { width: 0.05, height: 1, depth: 1 }, context.data.scene);
        vertical.renderingGroupId = 3;
        const horizontal = CreateBox(`${comp.node.index}-cross-hairs-horizonatal`, { width: 1, height: 0.05, depth: 1 }, context.data.scene);
