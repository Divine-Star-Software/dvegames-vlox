 ...
 * @returns
 */
const sweepAABBN = (physicsNodePosition, boundingBox, collisionNode, delta) => {
...
function applyTransform(position, body, state, deltaTime) {
    ...
    update(system) {
        ...
        for (let q = 0; q < system.queries.length; q++) {
            ...
            for (let i = 0; i < query.nodes.length; i++) {
                ...
                dataTool.setFocalPoint(0, position.x >> 0, position.y >> 0, position.z >> 0);
                const body = PhysicsBodyComponent.getRequired(node).schema;
                const collider = BoxColliderComponent.getRequired(node);
                const colliderState = PhysicsColliderStateComponent.getRequired(node).schema;
                boundingBox.update(collider.schema.size.x, collider.schema.size.y, collider.schema.size.z);
                ...
                for (let z = Math.floor(minZ); z <= Math.ceil(maxZ); z++) {
                    for (let x = Math.floor(minX); x <= Math.ceil(maxX); x++) {
                        ...
                        voxelPositon.z = z;
                        const voxel = dataTool.getVoxel(voxelPositon.x, voxelPositon.y, voxelPositon.z);
                        if (!voxel)
                            continue;
                ...
                while (true) {
                    ...
                    const minX = Math.floor(Math.min(position.x, previousPosiiton.x) - bboxHalfWidth);
                    const maxX = Math.floor(Math.max(position.x, previousPosiiton.x) + bboxHalfWidth);
                    const minY = Math.floor(Math.min(position.y, previousPosiiton.y) - bboxHalfHeight);
                    const maxY = Math.floor(Math.max(position.y, previousPosiiton.y) + bboxHalfHeight);
                    const minZ = Math.floor(Math.min(position.z, previousPosiiton.z) - bboxHalfDepth);
                    const maxZ = Math.floor(Math.max(position.z, previousPosiiton.z) + bboxHalfDepth);
                    aabb.results.reset();
                        ...
                        for (let z = minZ; z <= maxZ; z++) {
                            for (let x = minX; x <= maxX; x++) {
                                ...
                                for (const colliderNode of nodes) {
                                    ...
                                    const collisionCheck = sweepAABBN(start, boundingBox, colliderNode, delta);
                                    if (!voxel.checkCollisions() ||
                                        !voxel.getSubstanceData()["dve_is_solid"] ||
                                        !collider.isSolid)
                                        continue;
